...
scl["cloudpercent"] = scl["cloud"] / scl["sum"]

scl.to_csv("scl_aggr.csv")

for cl in [.5,.25,.1,.01]:
    cld = [el for el in scl.loc[scl["cloudpercent"]<=cl].index]
    outstr= "('"+ "', '".join(cld) + "')"

    filename="obs/observations_2016_lesseual_"+str(int(cl*100))+"clouds.txt"
