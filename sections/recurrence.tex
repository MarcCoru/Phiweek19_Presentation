...
\begin{frame}
	...
	\tikzstyle{focused} = [fill=tumorange, draw, rounded corners, thin]
	...
	\node[rec](rec){\gates{t}{unfocused}{unfocused}{unfocused}{unfocused}{unfocused}};
	\draw[conn] ($ (rec)+(0,5em) $) -- (rec) node[at start, above]{$\V{x}_t$};
	\draw[conn] (rec) -- ($ (rec)+(0,-5em) $) node[at end, below]{$\V{h}_t$};
	
	\draw[conn] (rec.south) to[in=270, out=250, looseness=2] ($ (rec.west)+(-1.5em,0) $) to[in=110, out=90, looseness=2] (rec.north) node[at start, left, xshift=-4em]{$\V{h}_{t-1}$};
	
	\node[rec, right=3em of rec](unrolled0){
		\only<1-2,4>{\gates{1}{unfocused}{unfocused}{unfocused}{unfocused}{unfocused}}
		\only<3>{\gates{1}{unfocused}{unfocused}{unfocused}{unfocused}{focused}}
	};
	\visible<1->{\node[above=of unrolled0, unfocused](x0){$\V{x}_1$};}
	\draw[conn] (x0) -- (unrolled0);
	\draw[conn] (unrolled0) -- ($ (unrolled0)+(0,-5em) $);
	\visible<1->{\node[below=of unrolled0, unfocused](h0){$\V{h}_1$};}
	...
	\node[rec, right=of unrolled0](unrolled1){
		\only<1>{\gates{2}{unfocused}{unfocused}{unfocused}{unfocused}{unfocused}}
		\only<2>{\gates{2}{focused}{unfocused}{focused}{unfocused}{unfocused}}
		\only<3>{\gates{2}{unfocused}{focused}{unfocused}{unfocused}{focused}}
		\only<4>{\gates{2}{unfocused}{unfocused}{unfocused}{focused}{focused}}
	};
	...
	\visible<1-3>{\draw[conn] (unrolled0) -- (unrolled1) node[midway, above, unfocused]{$\V{h}_1$};}
	\visible<4>{\draw[conn] (unrolled0) -- (unrolled1) node[midway, above, focused]{$\V{h}_1$};}
	
	\visible<1-3>{\draw[conn] (unrolled1) -- (unrolled2) node[midway, above, unfocused]{$\V{h}_2$};}
	\visible<4>{\draw[conn] (unrolled1) -- (unrolled2) node[midway, above, focused]{$\V{h}_2$};}
...
Long Short-Term Memory Neural Network (LSTM) calculates new output $\V{h}_t$ and cell state $\V{c}_t$ based on current input $\V{x}_t$ and previous (short-term) output $\V{h}_{t-1}$ and long-term internal cell state $\V{c}_{t-1}$ :
\begin{equation*}
